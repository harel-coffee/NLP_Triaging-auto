...
def fitnessFunctionCross(dataset, individuals):
    bestPop = []
    xTrain = dataset[:, 0:-1]
    yTrain = dataset[:, -1]
    ...
    for individual in individuals:
        ...
        for j in range(1, 100):
            clf = KNeighborsClassifier(n_neighbors=j, n_jobs=8,
                                       weights='distance')
            scores = cross_validate(clf, xTrain[:, items], yTrain, scoring='f1_macro', return_estimator=True, cv=skf)
            if np.amax(scores['test_score']) > bestScore:
                bestScore = np.amax(scores['test_score'])
                bestCLF = scores
        bestPop.append([individual, bestScore, bestCLF])
    bestPop = sorted(bestPop, key=lambda x: x[1], reverse=True)
    return bestPop

#optimise later by not training parents again
def fitnessFunction(trainingData, validationData, individuals):
    ...
    for individual in individuals:
        ...
        for j in range(1, 100):
            ...
            clf.fit(trainingData[:, items], trainingData[:, -1])#'Behandlungskategorie'])
            y_pred = clf.predict(validationData[:, items])
            score = f1_score(validationData[:, -1], y_pred, average='macro')#'Behandlungskategorie'], y_pred, average='macro')
            if score > bestScore:
                bestScore = score
                bestCLF = clf
        bestPop.append([individual, bestScore, bestCLF])
    ...
    for individual in individuals:
        ...
        for j in range(1, 100):
            ...
            for crossval in crossValPairs:
                ...
                #                            weights='distance',
                #                            metric=customMetric,
                #                            metric_params={'weights': w[:, np.append(items, -1)]})
                # scores = cross_validate(clf, xTrain[:, items], yTrain, scoring='f1_macro', return_estimator=True, cv=skf)
                # tempBest = np.amax(scores['test_score'])
                clf = KNeighborsClassifier(n_neighbors=j, n_jobs=8,
                                           weights='distance',
                                           metric='precomputed')
...
#mutate number of flags (max 87 min 1) and flags
...
def doTheStuff(train, val, name):
    savePath = '/gss/work/pofo5947/Triaging/models/knn/'#'C:/Users/michn/PycharmProjects/Triaging/models/knn/'
    ...
    for g in range(0, generations):
        ...
        # 2. evaluate fitness
        fitPop = fitnessFunction(train, val, population)
        print('Best specimen score: ' + str(fitPop[0][1]))
        # 3. select fittest
    ...
    try:
        print('Best specimen of process ' + str(processName) + ' has ' + str(len(bestIndividual[0])) +
            ' items with a F1-Score of ' + str(bestIndividual[1]) + '.')
    except Exception as exc:
        print(exc)
        print('Best specimen of process ' + str(processName) + ' has ' + str(np.array(bestIndividual[0]).shape) +
              ' items with a F1-Score of ' + str(bestIndividual[1]) + '.')
    joblib.dump(bestIndividual, savePath + 'bestSpecimen_late.dump')
    return

def doItRight(data, test, weights, name):
    savePath = '/gss/work/pofo5947/Triaging/models/knn/'  # 'C:/Users/michn/PycharmProjects/Triaging/models/knn/'
    processName = name#multiprocessing.current_process().name
    # 1. initialise generation
    population = []
    m = 200
    l = 100
    ...
    for g in range(0, generations):
        ...
        joblib.dump(fitPop[0], savePath + 'intermediateSpecimen_cv_cmetric_' + str(processName) + 'gen_' + str(g) + '.dump')
        # 3. select fittest
        if bestIndividual[1] < fitPop[0][1]:
            bestIndividual = fitPop[0]
            earlyStopping = 0
        ...
        # 5. strategy dependent: keep or omit
        population = offspring + parents
    joblib.dump(bestIndividual, savePath + 'bestSpecimen_cv_balanced_' + str(processName) + '.dump')
    #clfs = bestIndividual[2]['estimator'][np.argmax(bestIndividual[2]['test_score'])]
    #test_dist_matrix = customMetric(test[:, bestIndividual[0]], )
    #y_pred = clfs.predict(test[:, bestIndividual[0]])
    #test_score = f1_score(test[:, -1], y_pred, average='macro')
    #print('Test score: ' + str(test_score))
    try:
        print('Best specimen of process ' + str(processName) + ' has ' + str(len(bestIndividual[0])) +
              ' items with a ')
        print("F1-Score: %0.2f (+/- %0.2f)" % (
        bestIndividual[2]['test_score'].mean(), bestIndividual[2]['test_score'].std() * 2))
    except Exception as exc:
        print('Best specimen of process ' + str(processName) + ' has ' + str(np.array(bestIndividual[0]).shape) +
              ' items with a ')
        print("F1-Score: %0.2f (+/- %0.2f)" % (
...
basePath = '/gss/work/pofo5947/Triaging/data/dataset/'#'C:/Users/michn/PycharmProjects/Triaging/Data/dataset/'
...
doItRight(data, test, weights, param)
...
#     #train, val, test = dt.loadAsFrame(flags)
#     basePath = '/gss/work/pofo5947/Triaging/data/dataset/'#'C:/Users/michn/PycharmProjects/Triaging/Data/dataset/'
#     #savePath = 'C:/Users/michn/PycharmProjects/Triaging/models/knn/'#'/gss/work/pofo5947/Triaging/models/knn/'
#     #time.sleep(10*int(param))
